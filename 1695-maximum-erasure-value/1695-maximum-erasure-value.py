class Solution:
    def maximumUniqueSubarray(self, nums: List[int]) -> int:
        ...
        for i in range(1, len(nums)):
            nums[i] += nums[i-1]  # pre sum
        start = 0
        res = 0
        for j in range(len(nums)):
            num = (nums[j] if j == 0 else nums[j]-nums[j-1])
            start = max(start, hashtable[num]+1)
            res = max(res, nums[j]-(0 if start == 0 else nums[start-1]))
            hashtable[num] = j
...
#         for r in range(1,n):
#             print(l, nums[l],r, nums[r],lv[nums[r]])
#             maxs[r]=maxs[r-1]+nums[r]
#             if lv[nums[r]]>=l:
#                 maxs[r]-=maxs[lv[nums[r]]]
#                 l=lv[nums[r]]+1
#             lv[nums[r]]=r
#             print(l, nums[l],r, nums[r],lv[nums[r]])
#             print(maxs[r])
